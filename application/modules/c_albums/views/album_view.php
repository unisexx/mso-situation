...
<div class="line">&nbsp;</div>
      ...
      <ul>
          <?php foreach($pictures as $row): ?>
              <li>
                  <a href="uploads/c_albums/<?php echo $album->id?>/<?php echo $row->image?>" rel='lightbox[gal]' title="<?php echo $row->title?>"><?php echo thumb('uploads/c_albums/'.$album->id.'/'.$row->image,320,false,1,'')?></a>
              </li>
          <?php endforeach;?>
      </ul>
  </div>
