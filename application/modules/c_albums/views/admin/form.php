...
<script type="text/javascript">
	$(function(){
		...
		$('input[name=cat_add]').click(function(){
			...
			{
				$.post('c_albums/admin/c_albums/category_save',{'name':$('input[name=cat_name]').val()},function(data){
		...
		$('input[name=cat_edit]').click(function(){
			...
			{
				$.post('c_albums/admin/c_albums/category_save/'+ $('input[name=cat_edit_name]').attr('rel'),{'name':$('input[name=cat_edit_name]').val()},function(data){
					$('.album_category_id').html(data);
		...
		$('.cat_delete').click(function(){
			...
			{
				$.post('c_albums/admin/c_albums/category_delete/' + $('select[name=album_category_id] option:selected').val(),
				function(data){
					$('.album_category_id').html(data);
				})
			}
		...
		$('input[name=picture_delete]').click(function(){
			...
			{
				var me = $(this);
				$.post('c_albums/admin/c_albums/delete_picture/' + $(this).attr('rel'),function(data){
...
<form method="post" action="c_albums/admin/c_albums/save/<?php echo $album->id?>" enctype="multipart/form-data">
	<table class="form">
		...
		<?php foreach($album->c_picture->order_by('id','desc')->get() as $picture): ?>
		<tr>
			<th>รูปภาพ </th>
			<td>
				<a href="uploads/c_albums/<?php echo $album->id ?>/<?php echo $picture->image ?>" rel="lightbox[img]">
				<?php echo thumb('uploads/c_albums/'.$album->id.'/thumbnail/'.$picture->image,50,false,1,'style="vertical-align:middle;"')?>
				</a>
		...
		<tr>	
			...
			<td>
				...
				<?php echo form_button('',lang('btn_back'),'class="button" onclick="window.location = \'c_albums/admin/c_albums\'"')?>
			</td>
		</tr>
	</table>
</form>
